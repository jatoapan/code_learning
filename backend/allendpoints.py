...
def req(method, path, token="", data=None):
    ...
    try:
        ...
        try:
            ...
            res_json = body
        print(f"❌ {method} {path} -> HTTP {e.code} | {str(res_json)[:150]}")
        return {}
...
stu2_res = req("POST", "/users", data={"name":"Stu2", "email":"stu2@gmail.com", "password":"password123", "password_confirmation":"password123"})
